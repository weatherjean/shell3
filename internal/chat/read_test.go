...
func TestReadTool_WholeSmallFile(t *testing.T) {
	...
	out := handleReadTool(`{"path":"a.txt"}`, dir)
...
func TestReadTool_LineLimitFooter(t *testing.T) {
	...
	out := handleReadTool(`{"path":"big.txt"}`, dir)
...
func TestReadTool_Offset(t *testing.T) {
	...
	out := handleReadTool(`{"path":"n.txt","offset":3,"limit":1}`, dir)
...
func TestReadTool_BinaryRedirect(t *testing.T) {
	...
	out := handleReadTool(`{"path":"bin"}`, dir)
...
func TestReadTool_TinyBinaryFlagged(t *testing.T) {
	...
	out := handleReadTool(`{"path":"tiny.bin"}`, dir)
	if !strings.HasPrefix(out, "error:") || !strings.Contains(out, "read_media") {
...
func TestReadTool_TinyTextWithStrayByteNotFlagged(t *testing.T) {
	...
	out := handleReadTool(`{"path":"tiny.txt"}`, dir)
...
func TestReadTool_Errors(t *testing.T) {
	dir := t.TempDir()
	if out := handleReadTool(`{"path":"nope.txt"}`, dir); !strings.HasPrefix(out, "error:") || !strings.Contains(out, "not found") {
		t.Fatalf("missing-file error wrong: %q", out)
	}
	if out := handleReadTool(`{"path":"."}`, dir); !strings.HasPrefix(out, "error:") || !strings.Contains(out, "directory") {
...
func TestReadTool_ByteCapTruncatesBeforeLineCap(t *testing.T) {
	...
	out := handleReadTool(`{"path":"wide.txt"}`, dir)
	if !strings.Contains(out, "Use offset=") {
...
func TestReadTool_GiantSingleLine(t *testing.T) {
	...
	out := handleReadTool(`{"path":"one.txt"}`, dir)
...
func TestReadTool_OffsetBeyondEOF(t *testing.T) {
	...
	out := handleReadTool(`{"path":"s.txt","offset":99}`, dir)
...
func TestReadTool_EmptyFile(t *testing.T) {
	...
	if out := handleReadTool(`{"path":"empty.txt"}`, dir); out != "" {
...
func TestReadTool_NoTrailingNewline(t *testing.T) {
	...
	out := handleReadTool(`{"path":"nn.txt"}`, dir)
	...
	if out := handleReadTool(`{"path":"tn.txt"}`, dir); out != "alpha\nbeta\n" {
...
func TestReadTool_FileSizeCeiling(t *testing.T) {
	dir := t.TempDir()
	// A sparse file just over the in-memory ceiling: Stat reports the size but no
	// bytes are read (the guard returns before os.ReadFile).
	...
	f.Close()
	out := handleReadTool(`{"path":"huge.bin"}`, dir)
