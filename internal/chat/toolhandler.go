...
type ToolConfig struct {
	...
	// ReviewToolCall resolves a {review} soft deny: an LLM reviewer judges
	// command (bash tools only) given the gate's reason. Nil = no reviewer,
	// so a review verdict fails closed. Config-global.
