...
func TestDeliverReply_ModeAlwaysSendsVoice(t *testing.T) {
	...
	b.SetMedia(&MediaCaps{Clients: media.Clients{
		Speak: func(ctx context.Context, text string) (media.Speech, error) {
			return media.Speech{Path: writeSpeechFile(t, "out.ogg", []byte("opus-bytes"))}, nil
		},
	},
		TTSMode: "always",
	}, nil)
...
func TestDeliverReply_CleansSynthesizedFileAfterSendVoice(t *testing.T) {
	...
	b.SetMedia(&MediaCaps{Clients: media.Clients{
		Speak: func(ctx context.Context, text string) (media.Speech, error) {
			filePath = writeSpeechFile(t, "out.ogg", []byte("opus-bytes"))
			return media.Speech{Path: filePath}, nil
		},
	},
		TTSMode: "always",
	}, nil)
...
func TestDeliverReply_SendAudioFailsFallsBackToText(t *testing.T) {
	...
	b.SetMedia(&MediaCaps{Clients: media.Clients{
		Speak: func(ctx context.Context, text string) (media.Speech, error) {
			return media.Speech{Path: writeSpeechFile(t, "out.mp3", []byte("mp3-bytes"))}, nil
		},
	},
		TTSMode: "always",
	}, nil)
...
func TestDeliverReply_EmptyReplyNeverSpeaks(t *testing.T) {
	...
	b.SetMedia(&MediaCaps{Clients: media.Clients{
		Speak: func(ctx context.Context, text string) (media.Speech, error) {
			calls++
			return media.Speech{Path: writeSpeechFile(t, "out.ogg", []byte("x"))}, nil
		},
	},
		TTSMode: "always",
	}, nil)
...
func TestVoiceCommand_SetModePersistsAndConfirms(t *testing.T) {
	...
	rt, _ := newFakeRuntime(t, "ok")
	b := newBot(t, fc, rt)
	store := &ModeStore{Path: filepath.Join(t.TempDir(), "mode.json")}
	b.SetMedia(&MediaCaps{Clients: media.Clients{
		Speak: func(ctx context.Context, text string) (media.Speech, error) {
			return media.Speech{}, nil
		},
...
func TestHandleMsg_EndToEndVoiceReply(t *testing.T) {
	fc := newFakeClient()
	rt, _ := newFakeRuntime(t, "spoken reply")
	b := newBot(t, fc, rt)
	b.SetMedia(&MediaCaps{Clients: media.Clients{
		Speak: func(ctx context.Context, text string) (media.Speech, error) {
			return media.Speech{Path: writeSpeechFile(t, "out.ogg", []byte("opus-bytes"))}, nil
