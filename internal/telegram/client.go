...
type Msg struct {
	ChatID    int64
	ID        int // this message's Telegram message_id (0 if unknown)
	ReplyToID int // message_id this replies to, for thread resolution (0 = not a reply)
	Text      string
...
type tgClient interface {
	...
	// SendReply posts plain text as a Telegram reply to replyTo, threading it in
	// the chat. If replyTo no longer exists (deleted message) it falls back to a
	// plain Send so a thread whose anchor vanished never fails a turn.
	SendReply(ctx context.Context, chatID int64, text string, replyTo int) (msgID int, err error)
