---
name: self-evolve
description: How to safely change your own shell3.lua config or skills. Edit, validate with shell3 health, and the change applies on /reload or the next shell3 start. The running session is never affected by a bad edit.
---
...
4. Fix by error type: a Lua error names the broken line; `unknown model "y"` /
   `unknown subagent "x"` is a bad cross-reference — point it at a declared
   name; a `skill file ... skipped` warning means the `.md` needs a
   frontmatter `description` and a non-empty body. Re-run health until clean.
