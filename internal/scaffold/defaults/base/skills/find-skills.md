...
2. Tell the user plainly: this takes effect on a config reload (call your
   `reload` tool, or the user sends `/reload`), and it costs one line in
   every future prompt from now on.
...
- `shell3.yaml` and `hooks/*.sh` are the operator's, not yours to edit
  (self-evolve skill).
