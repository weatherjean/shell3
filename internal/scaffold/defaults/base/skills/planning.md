...
- Check `memory.md` for decisions and pitfalls already recorded — yours, and
  the employee's own if the work is inside its domain.
...
- Record durable outcomes in `memory.md` as usual; the plan file stays as the
  audit trail.
