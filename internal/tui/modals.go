...
func (m *model) helpBox() string {
	...
	lines := []string{
		...
		"",
		head.Render("input"),
		row("type", "the input is always live — no mode to leave"),
		row("enter", "send (queues as steering while a turn is busy)"),
		row("ctrl+j", "newline (shift+enter if supported)"),
		row("ctrl+o", "compose in $EDITOR"),
		row("ctrl+u", "clear input"),
		row("tab", "cycle agent"),
		row("pgup / pgdn", "scroll the transcript"),
		"",
		head.Render("mouse"),
		row("drag", "select + copy transcript lines"),
		row("click", "fold / unfold a tool or thinking block"),
		row("wheel", "scroll the transcript"),
		"",
		head.Render("global"),
		row("ctrl+p", "command palette"),
		row("?", "help (on an empty input)"),
		row("ctrl+c", "cancel turn / press again to quit"),
		"",
		head.Render("palette (ctrl+p)"),
		row("type", "filter commands"),
		row("↑ / ↓", "move selection"),
		row("enter", "run selected (or dispatch typed \"name arg\")"),
		row("tab", "complete selected name into the input"),
		row("esc", "close"),
		"",
