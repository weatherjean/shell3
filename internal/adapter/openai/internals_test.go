...
func TestToMessages_MixedParts(t *testing.T) {
	...
	// Verify part order is preserved: text, image_url, input_audio
	...
	if textIdx == -1 || imageIdx == -1 || audioIdx == -1 {
		t.Fatalf("not all parts found in: %s", s)
	}
	if textIdx >= imageIdx || imageIdx >= audioIdx {
		t.Fatalf("part order not preserved. textIdx=%d imageIdx=%d audioIdx=%d", textIdx, imageIdx, audioIdx)
	}
