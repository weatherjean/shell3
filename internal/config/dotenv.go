...
// comment only when outside quotes and preceded by whitespace; otherwise it is
// kept as a literal value character. In particular a `#` immediately after the
// `=` (KEY=#a1b2c3) is part of the value — secrets legitimately start with `#`,
// and truncating one here would silently substitute "" for it.
