...
// shell3MarkdownStyle recolors glamour's base theme to the active palette
...
// the light/dark sensing. The base is glamour's light or dark theme, chosen by
// the sensed terminal (activeLight) so glamour's own defaults (e.g. code-block
// chrome) suit it.
...
// resetMarkdown drops the width-keyed renderer cache and bumps mdEpoch so the
// next render picks up a palette change — both for new items and for ones
// already rendered under the old palette (applyPalette calls this).
func resetMarkdown() {
	mdMu.Lock()
