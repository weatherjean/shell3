...
that help output is the contract the agent reads before invoking it.`,
		Example: `  shell3 project new site --workdir ~/code/site --description "marketing site"
  shell3 project new api --workdir ~/code/api --copy-skills site`,
	...
	newCmd.Flags().StringVar(&f.workdir, "workdir", "", "Directory the project manager's shell runs in (must exist)")
	_ = newCmd.MarkFlagRequired("workdir")
