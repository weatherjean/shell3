...
- Background completion is restart- and outage-durable: persist before routing,
  delete only after successful handoff, and tolerate at-least-once delivery.
- `/stop` cancels a turn; `/superstop` additionally kills managed background
  commands and suppresses their manufactured completion posts.
...
completion routing, storage, or transport delivery require focused success and
