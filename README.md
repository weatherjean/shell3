...
pick it up (`systemctl --user restart shell3.service` when running as a
service). `shell3 --version` shows what you're on. Pin a release with
...
2. Run `shell3 boot` and fill in the form. It writes the config tree under
   `~/.shell3/` and, on Linux with systemd, offers to install a user service.
...
and answers exactly one `chat_id`. Full walkthrough in
...
- **Any OpenAI-compatible provider**: OpenAI, Ollama, Groq, OpenRouter,
  DeepSeek, and friends. MCP servers too, opt-in per agent and gated like
  every other tool.
