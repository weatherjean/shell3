...
it as dim reasoning rather than leaking `<think>` tags into the answer:
    ...
    shell3.model("minimax", {
      base_url       = "https://api.minimax.io/v1",
      api_key        = shell3.env.secret("MINIMAX_API_KEY"),  -- in .env beside shell3.lua
      model          = "MiniMax-M3",
      context_window = 128000,
      compact_at     = 100000,
      extra = {
        reasoning_split = true,  -- route thinking to reasoning_content, not inline <think> in content
      },
    })
...
## Other examples
    ...
    extra = { provider = { order = {"anthropic"} } }  -- OpenRouter provider routing (nested objects work too)
