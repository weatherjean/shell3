...
- **`cron { jobs = {…} }` block** in `shell3.lua`, parsed by `luacfg` into
  `[]CronJob{Schedule, Prompt, Agent, WorkDir}` on the loaded config.
...
cron = {
  jobs = {
    { schedule = "0 9 * * *",  agent = "explorer",
      prompt  = "Summarize my open PRs and anything that needs review today." },
    { schedule = "@hourly",    agent = "code",     workdir = "/path/to/repo",
...
2. **Dispatch.** The scheduler calls `mainSession.Dispatch(job.Agent,
   job.Prompt, {WorkDir: job.WorkDir, Label: "cron:<name>"})`.
...
4. **Report back.** On finish, `deliverSubagentResult` drops a labeled result
   (`[cron:<name>] …`) into the **main session inbox**.
...
- `DispatchOpts` shape: minimally `{WorkDir, Label}`; consider `{Timeout}` per
  job vs a global default.
