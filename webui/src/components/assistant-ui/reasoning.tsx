...
function ReasoningTrigger({
  ...
  duration?: number;
  /**
   * The opening of the thought, shown while the block is closed. A row of
   * identical "Reasoning" labels tells you nothing about which one to open;
   * the first line usually tells you whether you need to.
   */
  preview?: string;
}) {
  ...
  return (
    <CollapsibleTrigger
      ...
      className={cn(
        "aui-reasoning-trigger group/trigger text-ink-3 hover:text-ink-2 flex w-full origin-left items-baseline gap-2 py-1 font-serif text-[11.5px] italic transition-colors",
        className,
      )}
      {...props}
    >
      <ChevronDownIcon
        ...
        className="aui-reasoning-trigger-icon size-2.5 shrink-0 -rotate-90 transition-transform group-data-open/trigger:rotate-0 group-data-panel-open/trigger:rotate-0"
      />
      <span
      ...
      {preview ? (
        <span
          ...
          className={cn(
            "aui-reasoning-trigger-preview text-ink-3/80 min-w-0 flex-1 truncate",
            "text-start font-sans text-[11px] not-italic",
            // Redundant once the thought itself is on screen.
            "group-data-open/trigger:invisible group-data-panel-open/trigger:invisible",
          )}
        >
          {preview}
        </span>
