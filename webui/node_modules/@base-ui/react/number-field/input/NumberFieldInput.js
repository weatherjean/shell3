  ...
  const inputProps = {
    ...
    onKeyDown(event) {
      ...
      if (event.key === 'ArrowUp' || event.key === 'ArrowDown') {
        ...
        allowInputSyncRef.current = true;
        changed = setValue(min, (0, _createBaseUIEventDetails.createChangeEventDetails)(_reasons.REASONS.keyboard, nativeEvent));
      } else if (willSetEnd) {
        allowInputSyncRef.current = true;
        changed = setValue(max, (0, _createBaseUIEventDetails.createChangeEventDetails)(_reasons.REASONS.keyboard, nativeEvent));
      }
      if (changed) {
