import { unstable_formatInteractableSnapshot, unstable_getInteractableSnapshots } from "@assistant-ui/core";
//#region src/injectInteractableContext.ts
/**
...
* ```
*/
function unstable_injectInteractableContext(messages, format = unstable_formatInteractableSnapshot) {
	return messages.map((msg) => {
		if (msg.role !== "user") return msg;
		const items = unstable_getInteractableSnapshots(msg);
		if (!items?.length) return msg;
		const text = `${items.map(format).join("\n")}\n\n`;
		if (msg.parts?.[0]?.type === "text" && msg.parts[0].text === text) return msg;
		return {
			...msg,
			parts: [{
				type: "text",
...
//#endregion
export { unstable_injectInteractableContext };

//# sourceMappingURL=injectInteractableContext.js.map