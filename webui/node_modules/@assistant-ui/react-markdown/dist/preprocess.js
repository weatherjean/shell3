...
function rewriteLatexBracketDelimiters(text) {
	return text.replace(LATEX_INLINE_DELIMITER, (_, body) => `$${body.trim()}$`).replace(LATEX_DISPLAY_DELIMITER, (_, body) => `$$${body.trim()}$$`);
}
...
*/
function rewriteCustomMathTags(text) {
	return text.replace(MATH_TAG, (_, body) => `$$${body.trim()}$$`).replace(INLINE_TAG, (_, body) => `$${body.trim()}$`);
}
/**
